#!/usr/bin/env bash
# SessionStart hook: emit the always-on rules into the session's context. The
...
# unconditionally to every response *of the main loop*.
#
...
# is operator-facing ("install jq"), it is answered once per session rather than
# once per spawn, and a subagent can act on none of it.

set -u

"$(dirname "$0")/rules-payload.sh"
