...
- **`result=DIRTY`** / **`result=BEHIND`** / **`result=BLOCKED cause=terminal`** — a conflict with base, a base that has moved on (an "Update branch" click clears it), or a block nothing pending will clear. Surface the cause and ask; don't resolve a conflict autonomously. Neither prints a re-arm line: nothing about either state changes until a human acts, so a watch re-armed on one wakes on it again every tick. Re-spawn once the cause is dealt with.
