...
- **`result=DIRTY`** / **`result=BLOCKED cause=terminal`** — a conflict with base, or a block nothing pending will clear. Surface the cause and ask; don't resolve a conflict autonomously. Neither prints a re-arm line *on its own*: nothing about either state changes until a human acts, so a watch re-armed on one wakes on it again every tick. Re-spawn once the cause is dealt with. When the same tick also held activity, it arrives as that burst carrying `merge=DIRTY` — handle the round, re-arm, and the bare stop follows on the next tick.
