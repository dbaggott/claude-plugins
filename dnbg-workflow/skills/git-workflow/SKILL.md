...
**Do not hand-roll this loop** — the general case is the always-on rule "When shipped tooling doesn't fit, tell the user", which also says what to do when the shipped script genuinely doesn't cover yours: finish the task the narrow way, then tell the user and offer to report it. What follows is why *this* loop is one you especially don't want to rewrite. An earlier version of this skill inlined it here, and the inline version was wrong in ways that are not obvious until they bite:
