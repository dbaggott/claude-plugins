...
- The **stamp** records which version of these prompts made the claim, taken from the `## dnbg-workflow <version>` note injected at session start. It renders invisibly, and it sits on its own line so the `^Claimed by` grep above — which is line-anchored — cannot see it. **Drop the line entirely when no such note appeared**, which is the normal case rather than a fault: the stamp is opt-in and off unless the operator turns it on. A guessed version is worse than none, since nothing downstream can tell a guess from a reading.
